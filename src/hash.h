...
inline uint256 Hash(const T1 pbegin, const T1 pend)
{
    ...
    uint256 result;
    CHash256().Write(pbegin == pend ? pblank : (const uint8_t *)&pbegin[0], (pend - pbegin) * sizeof(pbegin[0]))
...
inline uint256 Hash(const T1 p1begin, const T1 p1end,
    ...
    static const uint8_t pblank[1] = {};
    uint256 result;
    CHash256().Write(p1begin == p1end ? pblank : (const uint8_t *)&p1begin[0], (p1end - p1begin) * sizeof(p1begin[0]))
...
inline uint160 Hash160(const T1 pbegin, const T1 pend)
{
    ...
    uint160 result;
    CHash160().Write(pbegin == pend ? pblank : (const uint8_t *)&pbegin[0], (pend - pbegin) * sizeof(pbegin[0]))
              .Finalize((uint8_t *)&result);
    return result;
}
...
void BIP32Hash(const ChainCode &chainCode, unsigned int nChild, uint8_t header, const uint8_t data[32], uint8_t output[64]);
